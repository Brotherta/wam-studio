 ...
 */
export function selectWithClass(element:Element, className:string, on_select:(element:Element)=>void = ()=>{}){
    const parent=element.parentElement
    if(parent){
        for(const child of parent.children){
            if(child.classList.contains(className)){
                on_select(child as HTMLElement)
 ...
 */
export function setupPannelMenus(base:ParentNode){
 ...
 */
export function waitForEvent<T extends keyof HTMLElementEventMap>(element:HTMLElement, event:T):Promise<HTMLElementEventMap[T]>{
    return new Promise<HTMLElementEventMap[T]>(resolve=>{
        const action=function(this:HTMLElement,e:HTMLElementEventMap[T]): any{
