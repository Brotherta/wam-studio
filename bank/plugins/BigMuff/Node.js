...
class BigMuffNode extends AudioWorkletNode {
  ...
  pitchWheel(channel, wheel) {
    for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
      ...
      this.setParamValue(
        pw.path,
        BigMuffNode.remap(wheel, 0, 16383, pw.min, pw.max),
      );
      if (this.output_handler) {
...
export default class BigMuff {
  ...
  async load() {
    try {
      ...
      let real_url =
        this.baseURL === "" ? "BigMuff.wasm" : this.baseURL + "/BigMuff.wasm";
      ...
      if (this.fWorkletProcessors.indexOf(name) === -1) {
        try {
          let re = /JSON_STR/g;
          let BigMuffProcessorString1 = BigMuffProcessorString.replace(
            re,
            json,
          );
          let real_url = window.URL.createObjectURL(
            new Blob([BigMuffProcessorString1], { type: "text/javascript" }),
          );
      ...
      this.node.onprocessorerror = () => {
        console.log("An error from BigMuff-processor was detected.");
      };
      return this.node;
    } catch (e) {
      console.error(e);
