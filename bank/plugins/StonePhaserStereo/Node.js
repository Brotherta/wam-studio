...
class StonePhaserStereoNode extends AudioWorkletNode {
  ...
  pitchWheel(channel, wheel) {
    for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
      ...
      this.setParamValue(
        pw.path,
        StonePhaserStereoNode.remap(wheel, 0, 16383, pw.min, pw.max),
      );
      if (this.output_handler) {
...
// Factory class
export default class StonePhaserStereo {
  ...
  async load() {
    try {
      ...
      let real_url =
        this.baseURL === ""
          ? "StonePhaserStereo.wasm"
          : this.baseURL + "/StonePhaserStereo.wasm";
      ...
      if (this.fWorkletProcessors.indexOf(name) === -1) {
        try {
          let re = /JSON_STR/g;
          let StonePhaserStereoProcessorString1 =
            StonePhaserStereoProcessorString.replace(re, json);
          let real_url = window.URL.createObjectURL(
            new Blob([StonePhaserStereoProcessorString1], {
