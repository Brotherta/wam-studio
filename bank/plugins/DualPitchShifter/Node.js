...
class DualPitchShifterNode extends AudioWorkletNode {
  ...
  pitchWheel(channel, wheel) {
    for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
      ...
      this.setParamValue(
        pw.path,
        DualPitchShifterNode.remap(wheel, 0, 16383, pw.min, pw.max),
      );
      if (this.output_handler) {
...
export default class DualPitchShifter {
  ...
  async load() {
    try {
      ...
      let real_url =
        this.baseURL === ""
          ? "DualPitchShifter.wasm"
          : this.baseURL + "/DualPitchShifter.wasm";
      ...
      if (this.fWorkletProcessors.indexOf(name) === -1) {
        try {
          let re = /JSON_STR/g;
          let DualPitchShifterProcessorString1 =
            DualPitchShifterProcessorString.replace(re, json);
          let real_url = window.URL.createObjectURL(
            new Blob([DualPitchShifterProcessorString1], {
