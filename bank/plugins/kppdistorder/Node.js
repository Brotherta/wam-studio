...
class kppdistorderNode extends AudioWorkletNode {
  ...
  pitchWheel(channel, wheel) {
    for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
      ...
      this.setParamValue(
        pw.path,
        kppdistorderNode.remap(wheel, 0, 16383, pw.min, pw.max),
      );
      if (this.output_handler) {
  ...
  async load() {
    try {
      ...
      let real_url =
        this.baseURL === ""
          ? "kppdistorder.wasm"
          : this.baseURL + "/kppdistorder.wasm";
      ...
      if (this.fWorkletProcessors.indexOf(name) === -1) {
        try {
          let re = /JSON_STR/g;
          let kppdistorderProcessorString1 =
            kppdistorderProcessorString.replace(re, json);
          let real_url = window.URL.createObjectURL(
            new Blob([kppdistorderProcessorString1], {
