...
class CompressorGuitarixNode extends AudioWorkletNode {
  ...
  pitchWheel(channel, wheel) {
    for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
      ...
      this.setParamValue(
        pw.path,
        CompressorGuitarixNode.remap(wheel, 0, 16383, pw.min, pw.max),
      );
      if (this.output_handler) {
...
class CompressorGuitarix {
  ...
  async load() {
    try {
      ...
      let real_url =
        this.baseURL === ""
          ? "CompressorGuitarix.wasm"
          : this.baseURL + "/CompressorGuitarix.wasm";
      ...
      if (this.fWorkletProcessors.indexOf(name) === -1) {
        try {
          let re = /JSON_STR/g;
          let CompressorGuitarixProcessorString1 =
            CompressorGuitarixProcessorString.replace(re, json);
          let real_url = window.URL.createObjectURL(
            new Blob([CompressorGuitarixProcessorString1], {
      ...
      this.node.onprocessorerror = () => {
        console.log("An error from CompressorGuitarix-processor was detected.");
      };
      return this.node;
    } catch (e) {
      console.error(e);
