...
class OctaverNode extends AudioWorkletNode {
  ...
  pitchWheel(channel, wheel) {
    for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
      ...
      this.setParamValue(
        pw.path,
        OctaverNode.remap(wheel, 0, 16383, pw.min, pw.max),
      );
      if (this.output_handler) {
...
class Octaver {
  ...
  async load() {
    try {
      ...
      let real_url =
        this.baseURL === "" ? "Octaver.wasm" : this.baseURL + "/Octaver.wasm";
      ...
      if (this.fWorkletProcessors.indexOf(name) === -1) {
        try {
          let re = /JSON_STR/g;
          let OctaverProcessorString1 = OctaverProcessorString.replace(
            re,
            json,
          );
          let real_url = window.URL.createObjectURL(
            new Blob([OctaverProcessorString1], { type: "text/javascript" }),
          );
      ...
      this.node.onprocessorerror = () => {
        console.log("An error from Octaver-processor was detected.");
      };
      return this.node;
    } catch (e) {
      console.error(e);
