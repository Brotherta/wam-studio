...
<div class="webaudio-switch-body" tabindex="1" touch-action="none" style="background-image: url(&quot;./img/switches/switch_1.png&quot;); background-size: 100% 200%; width: 55px; height: 31px; outline: none; background-position: 0px -100%;"><div class="webaudioctrl-tooltip" style="transition: opacity 0.1s ease 0s, visibility 0.1s ease 0s; opacity: 0; visibility: hidden;"></div></div>
  ...
  handleAnimationFrame = () => {
    this._root.getElementById("/Octaver/Down").value =
      this._plug.audioNode.getParamValue("/Octaver/Down");

    this._root.getElementById("/Octaver/Dry/Wet").value =
      this._plug.audioNode.getParamValue("/Octaver/Dry/Wet");

    this._root.getElementById("/Octaver/Up").value =
      this._plug.audioNode.getParamValue("/Octaver/Up");

    this._root.getElementById("/Octaver/bypass").value =
      1 - this._plug.audioNode.getParamValue("/Octaver/bypass");
  ...
  setKnobs() {
    this._root
      .getElementById("/Octaver/Down")
      .addEventListener("input", (e) =>
        this._plug.audioNode.setParamValue("/Octaver/Down", e.target.value),
      );
    this._root
      .getElementById("/Octaver/Dry/Wet")
      .addEventListener("input", (e) =>
        this._plug.audioNode.setParamValue("/Octaver/Dry/Wet", e.target.value),
      );
    this._root
      .getElementById("/Octaver/Up")
      .addEventListener("input", (e) =>
        this._plug.audioNode.setParamValue("/Octaver/Up", e.target.value),
      );
  }
...
try {
  customElements.define("wap-octaver", OctaverGui);
} catch (error) {
  console.log(error);
