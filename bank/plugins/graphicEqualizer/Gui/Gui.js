...
export default class GraphicEQHTMLElement extends HTMLElement {
  ...
  draw() {
    ...
    if (this.mode === "dragControlPoint") this.drawTooltip();
  ...
  drawGrid() {
    ...
    for (let octave = 0; octave + 1 <= this.noctaves; octave++) {
      ...
      if (octave !== 0) ctx.fillText(value + unit, x, 20);
      else ctx.fillText(value + unit, 10, 20);
    }
  ...
  findClosestFilterControlPoint(x, y) {
    for (let i = 0; i < this.plugin.audioNode.filters.length; i++) {
      ...
      const scale = this.getBoundingClientRect().width / this.offsetWidth;
      if (this.distance(x / scale, y / scale, cp.x, cp.y) < 5 / scale) {
