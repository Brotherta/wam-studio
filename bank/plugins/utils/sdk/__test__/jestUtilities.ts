...
type Expectation =
  | "toAllEqual"
  | "toAllBeGreaterThan"
  | "toAllBeGreaterThanOrEqual"
  | "toAllBeLessThan"
  | "toAllBeLessThanOrEqual"
  | "toAllIncrease"
  | "toAllDecrease";
...
function arrayExpectation(
  ...
  if (expectation === "toAllIncrease" || expectation === "toAllDecrease") {
    if (typeof expected !== "undefined")
      throw Error(
        '"expected" argument should not be present for "increase" or "decrease" validation.',
      );
  } else if (typeof expected !== "number" && !expected) {
    throw Error('Missing "expected" argument.');
  }
  let pass = true;
  let positiveString = "";
  let negativeString = "";
  ...
  if (!received.length)
    throw Error(
      `Can only call ${expectation} on non-empty Array-like objects.`,
    );
  if (typeof expected !== "number") {
    if (expected) {
      if (expected.length !== received.length)
        throw Error("Lengths must match when comparing to Array-like object.");
  ...
  const differenceString = jest.utils.diff(expected, received, {
    expand: jest.expand,
  });
  const resultString = pass
    ? jest.utils.matcherHint(`.not.${expectation}`)
    : jest.utils.matcherHint(`.${expectation}`);
  resultString.concat(
    "\n\n",
    `Expected all values of ${jest.utils.printReceived(received)}`,
  );
  resultString.concat(
    `${pass ? positiveString : negativeString} ${jest.utils.printReceived(expected)}`,
  );
...
expect.extend({
  toAllEqual(received, expected) {
    return arrayExpectation(this, "toAllEqual", received, expected);
  },
  toAllBeGreaterThanOrEqual(received, expected) {
  ...
  toAllBeGreaterThan(received, expected) {
    return arrayExpectation(this, "toAllBeGreaterThan", received, expected);
  },
  toAllBeLessThanOrEqual(received, expected) {
    return arrayExpectation(this, "toAllBeLessThanOrEqual", received, expected);
  },
  toAllBeLessThan(received, expected) {
    return arrayExpectation(this, "toAllBeLessThan", received, expected);
  },
  toAllIncrease(received) {
    return arrayExpectation(this, "toAllIncrease", received);
  },
  toAllDecrease(received) {
    return arrayExpectation(this, "toAllDecrease", received);
  },
});
