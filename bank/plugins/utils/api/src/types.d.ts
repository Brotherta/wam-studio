...
export const WebAudioModule: {
  ...
  createInstance<Node extends WamNode = WamNode>(
    groupId: string,
    audioContext: BaseAudioContext,
    initialState?: any,
  ): Promise<WebAudioModule<Node>>;
  /** WAM constructor, should call `initialize` after constructor to get it work */
  new <Node extends WamNode = WamNode>(
    groupId: string,
    audioContext: BaseAudioContext,
  ): WebAudioModule<Node>;
};

export type WamIODescriptor = Record<
  `has${"Audio" | "Midi" | "Sysex" | "Osc" | "Mpe" | "Automation"}${"Input" | "Output"}`,
  boolean
>;
...
export interface WamNode extends AudioNode {
  ...
  getParameterValues(
    normalized?: boolean,
    ...parameterIdQuery: string[]
  ): Promise<WamParameterDataMap>;
  ...
  addEventListener<K extends keyof WamEventMap>(
    type: K,
    listener: (
      this: this | AudioWorkletNode,
  ...
  addEventListener(
    type: string,
    listener: (this: this | AudioWorkletNode, ev: CustomEvent) => any,
    options?: boolean | AddEventListenerOptions,
  ): void;
  addEventListener(
    type: string,
  ...
  removeEventListener<K extends keyof WamEventMap>(
    type: K,
    listener: (
      this: this | AudioWorkletNode,
      ev: CustomEvent<WamEventMap[K]>,
...
export interface WamParameterInfo
  extends Readonly<Required<WamParameterConfiguration>> {
...
export type WamEventCallback<E extends WamEventType = WamEventType> = (
  event: WamEventMap[E],
) => any;
...
export type WamEvent =
  ...
  | WamOscEvent
  | WamInfoEvent;
export type WamAutomationEvent = WamEventBase<
  "wam-automation",
  WamParameterData
>;
