...
const processor = (moduleId, paramsConfig) => {
  ...
  const { AudioWorkletProcessor, registerProcessor, webAudioModules } =
  ...
  const normalize = (x, min, max) =>
    min === 0 && max === 1 ? x : (x - min) / (max - min) || 0;
  const denormalize = (x, min, max) =>
    min === 0 && max === 1 ? x : x * (max - min) + min;
  const mapValue = (x, eMin, eMax, sMin, sMax, tMin, tMax) =>
    denormalize(
      normalize(
        normalize(Math.min(sMax, Math.max(sMin, x)), eMin, eMax),
  ...
  class ParamMgrProcessor extends AudioWorkletProcessor {
    static get parameterDescriptors() {
      return Object.entries(paramsConfig).map(
    ...
    constructor(options) {
      ...
      this.$internalParamsBuffer = new Float32Array(
        this.buffer,
        4,
        this.internalParamsCount,
      );
      /** @type {WamEvent[]} */
      this.eventQueue = [];
    ...
    getParameterValues(normalized, ...parameterIdQuery) {
      ...
      parameterIdQuery.forEach((parameterId) => {
        ...
        parameterValues[parameterId] = {
          id: parameterId,
          value: normalized
            ? normalizeE(value, minValue, maxValue, exponent)
            : value,
          normalized,
        };
      });
    ...
    scheduleEvents(...events) {
      ...
      this.eventQueue.sort(
        (a, b) => (a.time || currentTime) - (b.time || currentTime),
      );
    }
    ...
    process(inputs, outputs, parameters) {
      ...
      this.lock();
      Object.entries(this.paramsConfig).forEach(
        ([name, { minValue, maxValue }]) => {
          const raw = parameters[name];
          if (name in this.paramsValues)
            this.paramsValues[name] = raw[raw.length - 1]; // Store to local temporary
          if (!this.paramsMapping[name]) return; // No need to output
          Object.entries(this.paramsMapping[name]).forEach(
            ([targetName, targetMapping]) => {
    ...
    disconnectEvents(wamInstanceId, output) {
      ...
      webAudioModules.disconnectEvents(
        this.groupId,
        this.instanceId,
        wamInstanceId,
        output,
