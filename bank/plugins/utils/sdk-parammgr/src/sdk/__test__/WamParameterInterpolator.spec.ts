...
describe("WamParameterInterpolator Suite", () => {
  ...
  const startValue = -10;
  const endValue = 10;
  const infoA = new WamParameterInfo("A", {
    defaultValue: 0.5,
    minValue: startValue,
    maxValue: endValue,
  });
  const infoB = new WamParameterInfo("B", {
    defaultValue: 3.0,
    minValue: startValue,
    maxValue: endValue,
  });
  ...
  it("Should only be done when filled with end value", () => {
    ...
    const filledSlice = testA.values.slice(
      samplesPerInterpolation + 1,
      samplesPerRenderQuantum,
    );
    // Verify values
  ...
  it("Should work properly when interpolation period is greater than that of render quantum", () => {
    const longerInterpolationPeriod = Math.round(
      samplesPerRenderQuantum * Math.PI,
    );
    const infoD = new WamParameterInfo("D", {
      defaultValue: startValue,
      minValue: startValue,
      maxValue: endValue,
    });
    const testD = new WamParameterInterpolator(
      infoD,
      longerInterpolationPeriod,
    );
    ...
    let fullRenders = Math.floor(
      longerInterpolationPeriod / samplesPerRenderQuantum,
    );
    while (fullRenders--) {
    ...
    expect(testD.values.slice(remainder, samplesPerRenderQuantum)).toAllEqual(
      endValue,
    );
