...
describe("WamEventRingBuffer Suite", () => {
  ...
  it("Should handle updating parameter ids", () => {
    ...
    const sab = WamEventRingBuffer.getStorageForEventCapacity(
      RingBuffer,
      inputEvents.length,
    );
    const test = new WamEventRingBuffer(
      RingBuffer,
      sab,
      parameterIds,
      binaryBytesLength,
    ...
    expectedOutputEvents = [yAutomationEvent, zAutomationEvent];
  ...
  it("Should allocate enough bytes for events of all types", () => {
    const defaultSab = WamEventRingBuffer.getStorageForEventCapacity(
      RingBuffer,
      1,
    );
    expect(defaultSab.byteLength).toBeGreaterThan(
      WamEventRingBuffer.WamEventBaseBytes,
    );
    expect(defaultSab.byteLength).toBeGreaterThan(
    ...
    const binarySab = WamEventRingBuffer.getStorageForEventCapacity(
      RingBuffer,
      1,
      binaryBytesLength,
    );
    expect(binarySab.byteLength).toBeGreaterThan(
      WamEventRingBuffer.WamBinaryEventBytes + binaryBytesLength,
    );

    const testNumEvents = 333;
    const testSab = WamEventRingBuffer.getStorageForEventCapacity(
      RingBuffer,
      testNumEvents,
    );
    expect(testSab.byteLength).toBeGreaterThan(
      WamEventRingBuffer.WamTransportEventBytes * testNumEvents,
    );
  });

  it("Should only write number of events for which there is enough space", () => {
    const inputEvents = [inputSysexEvent, inputSysexEvent];
