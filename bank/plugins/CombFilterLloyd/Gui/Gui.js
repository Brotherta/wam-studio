...
<div class="webaudio-switch-body" tabindex="1" touch-action="none" style="background-image: url(&quot;./img/switches/switch_1.png&quot;); background-size: 100% 200%; width: 64px; height: 40px; outline: none; background-position: 0px -100%;"><div class="webaudioctrl-tooltip" style="transition: opacity 0.1s ease 0s, visibility 0.1s ease 0s; opacity: 0; visibility: hidden;"></div></div>
      ...
      // check if the shadowroot host has a background image
        ...
        this._root.getElementById('/CombFilterLloyd/Intdel').value = this._plug.audioNode.getParamValue('/CombFilterLloyd/Intdel');
        

        this._root.getElementById('/CombFilterLloyd/aN').value = this._plug.audioNode.getParamValue('/CombFilterLloyd/aN');
        

          this._root.getElementById('/CombFilterLloyd/bypass').value = 1 - this._plug.audioNode.getParamValue('/CombFilterLloyd/bypass');
...
window.requestAnimationFrame(this.handleAnimationFrame);
        ...
        return ['state'];
              ...
              setKnobs() {
                 this._root.getElementById("/CombFilterLloyd/Intdel").addEventListener("input", (e) =>this._plug.audioNode.setParamValue("/CombFilterLloyd/Intdel", e.target.value));
this._root.getElementById("/CombFilterLloyd/aN").addEventListener("input", (e) =>this._plug.audioNode.setParamValue("/CombFilterLloyd/aN", e.target.value));
              ...
              setSwitches() {
                 this._root.getElementById("/CombFilterLloyd/bypass").addEventListener("change", (e) =>this._plug.audioNode.setParamValue("/CombFilterLloyd/bypass", 1 - e.target.value));
      ...
      try {
          customElements.define('wap-combfilterlloyd', 
                                CombFilterLloydGui);
