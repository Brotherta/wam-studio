import {
  WamProcessor,
  AudioWorkletGlobalScope as IAudioWorkletGlobalScope,
} from "@webaudiomodules/api";

export interface TypedAudioWorkletNodeOptions<T = any>
...
export type TypedEventListenerOrEventListenerObject<EventDetail = any> =
  | TypedEventListener<EventDetail>
  | TypedEventListenerObject<EventDetail>;

export interface TypedEventTarget<EventMap extends Record<string, any> = any>
  extends EventTarget {
...
export interface TypedMessagePort<In = any, Out = any> extends MessagePort {
  onmessage:
    | ((this: TypedMessagePort<In, Out>, ev: TypedMessageEvent<In>) => any)
    | null;
  onmessageerror:
    | ((this: TypedMessagePort<In, Out>, ev: TypedMessageEvent<In>) => any)
    | null;
  ...
  addEventListener<K extends keyof TypedMessagePortEventMap<In>>(
    type: K,
    listener: (this: MessagePort, ev: TypedMessagePortEventMap<In>[K]) => any,
    options?: boolean | AddEventListenerOptions,
  ): void;
  addEventListener(
    type: string,
  ...
  removeEventListener<K extends keyof TypedMessagePortEventMap<In>>(
    type: K,
    listener: (this: MessagePort, ev: TypedMessagePortEventMap<In>[K]) => any,
    options?: boolean | EventListenerOptions,
  ): void;
...
export interface TypedAudioParamDescriptor<Par extends string = string>
  extends AudioParamDescriptor {
  automationRate?: AutomationRate;
  defaultValue?: number;
  maxValue?: number;
  minValue?: number;
...
export interface TypedAudioWorkletProcessor<
  MsgIn = any,
  MsgOut = any,
...
> {
  ...
  process(
    ...
    parameters: Record<Par, Float32Array>,
  ): boolean;
}
export const TypedAudioWorkletProcessor: {
  parameterDescriptors: TypedAudioParamDescriptor[];
  new <MsgIn = any, MsgOut = any, Par extends string = string, Opt = any>(
    options: TypedAudioWorkletNodeOptions<Opt>,
  ): TypedAudioWorkletProcessor<MsgIn, MsgOut, Par>;
};
...
export type TypedAudioParamMap<P extends string = string> = ReadonlyMap<
  P,
  AudioParam
>;

export interface TypedAudioWorkletNode<
  MsgIn = any,
  MsgOut = any,
  Par extends string = string,
...
> extends AudioWorkletNode {
  ...
  addEventListener<K extends keyof EventMap>(
    type: K,
    listener: (this: AudioWorkletNode, ev: CustomEvent<EventMap[K]>) => any,
    options?: boolean | AddEventListenerOptions,
  ): void;
  addEventListener(
    type: string,
...
export const TypedAudioWorkletNode: {
  ...
  >(
    ...
    name: string,
    options?: TypedAudioWorkletNodeOptions<Opt>,
  ): TypedAudioWorkletNode<MsgIn, MsgOut, Par, EventMap>;
};
