...
const getWamProcessor = (moduleId) => {
  ...
  class WamProcessor extends AudioWorkletProcessor {
    ...
    constructor(options) {
      super(options);
      const { groupId, moduleId, instanceId, useSab } =
    ...
    _initialize() {
      ...
      Object.keys(this._parameterInfo).forEach((parameterId) => {
        ...
        this._parameterState[parameterId] = new WamParameter(
          this._parameterInfo[parameterId],
        );
        this._parameterInterpolators[parameterId] =
          new WamParameterInterpolator(info, 256);
      });
    }
      ...
      if (message.data.request) {
        ...
        if (verb === "get") {
          if (noun === "parameterInfo") {
            ...
            response.content = this._getParameterValues(
              normalized,
              parameterIds,
            );
          } else if (noun === "state") {
          ...
          if (noun === "processor") {
            ...
            this._eventWriter = new WamEventRingBuffer(
              RingBuffer,
              this._audioToMainEventSab,
              parameterIds,
            );
            /** @private @type {WamEventRingBuffer} */
            this._eventReader = new WamEventRingBuffer(
              RingBuffer,
              this._mainToAudioEventSab,
              parameterIds,
            );
    ...
    _interpolateParameterValues(startIndex, endIndex) {
      ...
      let i = 0;
      while (i < parameterIds.length) {
        this._parameterInterpolators[parameterIds[i]].process(
          startIndex,
          endIndex,
        );
        i++;
    ...
    _getProcessingSlices() {
      ...
      while (i < this._eventQueue.length) {
        ...
        const sampleIndex =
          offsetSec > 0 ? Math.round(offsetSec * sampleRate) : 0;
      ...
      while (i < keys.length) {
        ...
        const endSample =
          i < lastIndex ? parseInt(keys[i + 1]) : this._samplesPerQuantum;
