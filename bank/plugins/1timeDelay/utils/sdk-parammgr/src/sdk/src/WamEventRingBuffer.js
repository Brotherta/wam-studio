...
const getWamEventRingBuffer = (moduleId) => {
  ...
  class WamEventRingBuffer {
     ...
     * @type {number}
     */
    static WamAutomationEventBytes =
      WamEventRingBuffer.WamEventBaseBytes + 2 + 8 + 1;
     ...
     * @type {number}
     */
    static WamTransportEventBytes =
      WamEventRingBuffer.WamEventBaseBytes + 4 + 8 + 8 + 1 + 1 + 1;
     ...
     */
    static getStorageForEventCapacity(
      RingBuffer,
      eventCapacity,
      maxBytesPerEvent = undefined,
    ) {
      if (maxBytesPerEvent === undefined)
        maxBytesPerEvent = WamEventRingBuffer.DefaultExtraBytesPerEvent;
      else
        maxBytesPerEvent = Math.max(
          maxBytesPerEvent,
          WamEventRingBuffer.DefaultExtraBytesPerEvent,
        );
      const capacity =
        (Math.max(
    ...
    constructor(RingBuffer, sab, parameterIds, maxBytesPerEvent = undefined) {
      ...
      wamEventTypes.forEach((type, encodedType) => {
        ...
        switch (type) {
          case "wam-automation":
            byteSize = WamEventRingBuffer.WamAutomationEventBytes;
            break;
          case "wam-transport":
            byteSize = WamEventRingBuffer.WamTransportEventBytes;
            break;
          case "wam-mpe":
          case "wam-midi":
            byteSize = WamEventRingBuffer.WamMidiEventBytes;
            break;
          case "wam-osc":
          case "wam-sysex":
          case "wam-info":
            byteSize = WamEventRingBuffer.WamBinaryEventBytes;
            break;
          default:
            break;
        }
    ...
    _writeHeader(byteSize, type, time) {
      ...
      byteOffset += 1;
      this._eventBytesView.setFloat64(
        byteOffset,
        Number.isFinite(time) ? time : -1,
      );
      byteOffset += 8;
      return byteOffset;
    }
        ...
        case "wam-info":
          {
            ...
            if (bytesRequired > this._eventBytesAvailable)
              console.error(
                `Event requires ${bytesRequired} bytes but only ${this._eventBytesAvailable} have been allocated!`,
              );
    ...
    _decode() {
      let byteOffset = 0;
      const type =
        this._decodeEventType[this._eventBytesView.getUint8(byteOffset)];
      byteOffset += 1;
    ...
    read() {
      ...
      while (bytesAvailable > 0) {
        ...
        const eventBytes = new Uint8Array(
          this._eventBytes,
          0,
          eventSizeBytes - 4,
        );
  ...
  if (audioWorkletGlobalScope.AudioWorkletProcessor) {
    ...
    if (!ModuleScope.WamEventRingBuffer)
      ModuleScope.WamEventRingBuffer = WamEventRingBuffer;
  }
