...
export class AudioWorkletRegister {
  ...
  static register(
    moduleId: string,
    processor: (id: string, ...injections: any[]) => void,
...
export type InternalParametersDescriptor<
  InternalParams extends string = string,
> = Record<InternalParams, AudioParam | InternalParameterDescriptor>;
...
export type ParametersMapping<
  ...
  InternalParams extends string = string,
> = Record<Params, Record<InternalParams, ParameterMappingTarget>>;
...
export type PromisifiedFunction<F extends (...args: any[]) => any> = (
  ...args: Parameters<F>
) => PromiseLike<ReturnType<F>>;

export type UnPromisifiedFunction<F extends (...args: any[]) => any> = (
  ...args: Parameters<F>
) => ReturnType<F> extends PromiseLike<infer P> ? P : ReturnType<F>;
...
export type UnPromisifiedFunctionMap<T> = {
  [K in keyof T]: T[K] extends (...args: any[]) => any
    ? UnPromisifiedFunction<T[K]>
    : T[K];
};
export interface MessagePortRequest<
...
export interface ParamMgrCallToProcessor
  extends UnPromisifiedFunctionMap<
    Pick<
      WamNode,
      | "destroy"
...
export interface ParamMgrOptions
  extends TypedAudioWorkletNodeOptions<ParamMgrAudioWorkletOptions> {
...
export interface MgrAudioParam extends AudioParam, WamParameter {
  ...
  exponentialRampToNormalizedValueAtTime(
    value: number,
    endTime: number,
  ): MgrAudioParam;
  linearRampToValueAtTime(value: number, endTime: number): MgrAudioParam;
  linearRampToNormalizedValueAtTime(
    value: number,
    endTime: number,
  ): MgrAudioParam;
  setTargetAtTime(
...
export interface ParamMgrNodeMsgIn
  extends MessagePortResponse<ParamMgrCallToProcessor>,
    MessagePortRequest<ParamMgrCallFromProcessor> {}
export interface ParamMgrNodeMsgOut
  extends MessagePortRequest<ParamMgrCallToProcessor>,
    MessagePortResponse<ParamMgrCallFromProcessor> {}
 ...
 */
export interface ParamMgrNode<
  Params extends string = string,
  InternalParams extends string = Params,
> extends TypedAudioWorkletNode<
      ParamMgrNodeMsgIn,
      ParamMgrNodeMsgOut,
      Params,
      WamEventMap
    >,
    Omit<WamNode, keyof AudioWorkletNode>,
  ...
  linearRampToNormalizedParamValueAtTime(
    name: Params,
    value: number,
    endTime: number,
  ): AudioParam;
  ...
  exponentialRampToNormalizedParamValueAtTime(
    name: Params,
    value: number,
    endTime: number,
  ): AudioParam;
  setParamTargetAtTime(
    name: Params,
    target: number,
    startTime: number,
    timeConstant: number,
  ): AudioParam;
  setNormalizedParamTargetAtTime(
    name: Params,
    target: number,
    startTime: number,
    timeConstant: number,
  ...
  setParamValueCurveAtTime(
    name: Params,
    values: Iterable<number> | number[] | Float32Array,
    startTime: number,
    duration: number,
  ): AudioParam;
  setNormalizedParamValueCurveAtTime(
    name: Params,
    values: Iterable<number> | number[] | Float32Array,
    startTime: number,
    duration: number,
...
export const ParamMgrFactory: {
  ...
  >(
    module: WebAudioModule,
    optionsIn?: ParametersMappingConfiguratorOptions<Params, InternalParams>,
  ): Promise<ParamMgrNode<Params, InternalParams>>;
};
