...
router.get("/projects", (req, res) => {
  ...
  res.json(
    projects.map(({ username, name, date, id }) => ({
      username,
      name,
      date,
      id,
...
router.get("/projects/search", (req, res) => {
  ...
  const filteredProjects = projects.filter(({ username, name }) => {
    ...
    return (
      (lowerUsernameQuery === "" ||
        lowerUsername.includes(lowerUsernameQuery)) &&
      (lowerProjectNameQuery === "" ||
        lowerName.includes(lowerProjectNameQuery))
    );
  });

  res.json(
    filteredProjects.map(({ username, name, date, id }) => ({
      username,
      name,
      date,
      id,
