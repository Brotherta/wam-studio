...
export default class ScrollBarElement extends HTMLElement {
    ...
    customScrollTo(scroll: number) {
        const newValue = Math.max(0, Math.min(this.value + scroll, this.worldSize - this.size));
    ...
    bindEvents() {
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            let delta = this.orientation === 'vertical' ? (e.clientY - this.dragStart) : (e.clientX - this.dragStart);
            let trackSize = this.orientation === 'vertical' ? track.offsetHeight : track.offsetWidth;
            let handleSize = this.orientation === 'vertical' ? handle.offsetHeight : handle.offsetWidth;

            // Translate delta pixel of the client movement into value change depending on the world size
            let valueChange = (delta / (trackSize - handleSize)) * (this.worldSize - this.size);

            if (isNaN(valueChange)) return; // If the handle is the same size as the track, don't do anything
            this.value = Math.max(0, Math.min(this.startValue + valueChange, this.worldSize - this.size));
    ...
    updateHandlePosition(type = 'default') {
        ...
        if (this.size === this.worldSize) {
            ...
            this.handlePos = this.value / (this.worldSize - this.size) * (trackSize - handleSize);
        }
